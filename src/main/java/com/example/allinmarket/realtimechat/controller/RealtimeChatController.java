...
public class RealtimeChatController {
    ...
    public void message(RealtimeChatMessageDto dto) {
        if (RealtimeMessageType.ENTER.equals(dto.type())) {
            // 입장 메세지 (필요시 구현)
        }
        // ChatService를 호출하여 DB(chat_messages 테이블)에 저장하는 로직 추가
        RealtimeChatMessage saved = realtimeChatService.save(dto);
    ...
    public void read(RealtimeReadDto dto) {
        realtimeChatService.read(dto.roomId(), dto.userId(), dto.lastReadMessageId());
    }
}

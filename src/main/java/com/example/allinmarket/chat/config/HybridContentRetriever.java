...
public class HybridContentRetriever implements ContentRetriever{
    ...
    public List<Content> retrieve(Query query) {
        log.info("[Hybrid] query length={}", query.text().length());
        ...
        for (int i = 0; i < vectorResults.size(); i++) {
            ...
            log.debug("[RRF-Vector] 순위: {}, 점수: {}", i + 1, String.format(Locale.ROOT, "%.4f", score));
        }
        ...
        List<Content> result = rrfScores.entrySet().stream()
                ...
                .peek(entry -> log.debug("[RRF-Final] 최종 점수: {}", String.format(Locale.ROOT, "%.4f", entry.getValue())))

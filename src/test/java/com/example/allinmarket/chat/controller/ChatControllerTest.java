...
// SecurityConfig를 포함해야 springSecurityFilterChain 빈이 생성되고
// springSecurity()가 정상 동작합니다.
@WebMvcTest({ChatController.class, GlobalExceptionHandler.class, SecurityConfig.class})
class ChatControllerTest extends RestDocsControllerTest {
    ...
    // ────────────────────────────────────────────────────
    // RestDocsControllerTest의 setUpRestDocs()를 오버라이드해서
    // 이 테스트 클래스에서만 springSecurity()를 추가 적용합니다.
    // 다른 테스트 클래스는 RestDocsControllerTest 원본 그대로 사용해
    // springSecurityFilterChain 오류가 발생하지 않습니다.
    // ────────────────────────────────────────────────────

    @BeforeEach
    void setUpWithSecurity(RestDocumentationContextProvider restDocumentation) throws Exception {
    ...
    // SecurityContext 헬퍼
    //
    // UserPrincipal을 직접 넣고 ROLE_BUYER 권한도 함께 설정해야
    // SecurityConfig의 anyRequest().hasRole("BUYER")를 통과합니다.
    // ────────────────────────────────────────────────────

    private void setAuthContext(Long userId) {
        UserPrincipal userPrincipal = new UserPrincipal(userId, RealtimeChatSenderType.BUYER, "test-token");
    ...
    // TokenStream 목 헬퍼
    //
    // evaluate()는 내부에 CountDownLatch가 있어서
    // start() 호출 시점에 onCompleteResponse 콜백이
    // 즉시 실행되어야 latch가 열립니다.
    // langchain4j 1.x: onCompleteResponse의 콜백 타입이
    // Response<AiMessage> → ChatResponse로 변경되었습니다.
    // ────────────────────────────────────────────────────
    ...
    private TokenStream mockTokenStream(String fullText) {
        ...
        given(ts.onPartialResponseWithContext(any())).willReturn(ts);
        given(ts.onCompleteResponse(any())).willAnswer(invocation -> {
            Consumer<ChatResponse> cb = invocation.getArgument(0);
            cb.accept(ChatResponse.builder()
                    .aiMessage(AiMessage.from(fullText))
                    .build());
            return ts;
        });
        given(ts.onError(any())).willReturn(ts);
    ...
    // ────────────────────────────────────────────────────
    // POST /chat/evaluate (RestDocs 문서화 포함)
    // ────────────────────────────────────────────────────
    ...
    class EvaluateTest {
        ...
        @Test
        @DisplayName("INQUIRY 의도 - RAG 검색 후 답변 생성 성공 (RestDocs)")
        void evaluate_INQUIRY_성공() throws Exception {
            setAuthContext(1L);

            given(moderationService.isFlagged(anyString())).willReturn(false);
            given(intentClassifier.classify(anyString())).willReturn("INQUIRY");

            Content mockContent = mock(Content.class);
            dev.langchain4j.data.segment.TextSegment segment =
                    dev.langchain4j.data.segment.TextSegment.from("반품은 7일 이내 가능합니다.");
            given(mockContent.textSegment()).willReturn(segment);
            given(contentRetriever.retrieve(any(Query.class))).willReturn(List.of(mockContent));

            TokenStream ts = mockTokenStream("반품은 구매일로부터 7일 이내에 신청하실 수 있습니다.");
            given(aiAssistant.chat(anyLong(), anyString())).willReturn(ts);
            ...
            mockMvc.perform(post("/chat/evaluate")
                    ...
                    .andDo(document("chat/evaluate/inquiry",
                            ...
                            responseFields(
                                    fieldWithPath("answer").description("AI 답변"),
                                    fieldWithPath("contexts").description("RAG로 검색된 정책 문서 컨텍스트 목록")
                            )
                    ));
        }

        @Test
        @DisplayName("SMALL_TALK 의도 - RAG 검색 없이 일상 대화 답변 성공 (RestDocs)")
        void evaluate_SMALL_TALK_성공() throws Exception {
            setAuthContext(1L);

            given(moderationService.isFlagged(anyString())).willReturn(false);
            given(intentClassifier.classify(anyString())).willReturn(ChatConsts.SMALL_TALK);

            TokenStream ts = mockTokenStream("안녕하세요! 무엇을 도와드릴까요?");
            given(aiAssistant.smallTalk(anyLong(), anyString())).willReturn(ts);
            ...
            mockMvc.perform(post("/chat/evaluate")
                    ...
                    .andDo(document("chat/evaluate/small-talk",
                            ...
                            responseFields(
                                    fieldWithPath("answer").description("AI 답변"),
                                    fieldWithPath("contexts").description("RAG 컨텍스트 (일상 대화의 경우 빈 배열)")
                            )
                    ));
        }

        @Test
        @DisplayName("유해 콘텐츠 감지 시 부적절한 내용 응답 반환 (RestDocs)")
        void evaluate_유해_콘텐츠_감지() throws Exception {
            setAuthContext(1L);

            given(moderationService.isFlagged(anyString())).willReturn(true);
            ...
            mockMvc.perform(post("/chat/evaluate")
                    ...
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.answer").value("[부적절한 내용]"))
                    .andExpect(jsonPath("$.contexts").isArray())
                    .andExpect(jsonPath("$.contexts.length()").value(0))
                    .andDo(document("chat/evaluate/flagged",
                            ...
                            responseFields(
                                    fieldWithPath("answer").description("부적절한 내용 안내 메시지"),
                                    fieldWithPath("contexts").description("빈 배열")
                            )
                    ));

            verify(moderationService).isFlagged(anyString());
        }
